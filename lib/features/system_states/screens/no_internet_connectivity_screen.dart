                ...
                child: ListView(
                  children: [
                    Container(
                      ...
                      child: Column(children: [const CircleAvatar(radius: 34, backgroundColor: Color(0xFFEAF1FD), child: Icon(Icons.wifi_off_rounded, size: 30, color: AppColors.skyBlue)), const SizedBox(height: 18), Text('Connection Lost', style: theme.textTheme.headlineMedium?.copyWith(color: AppColors.trustNavy, fontWeight: FontWeight.w700)), const SizedBox(height: 12), Text('SafeWalk is currently offline. Don\'t worry, your SOS local triggers are still active.', textAlign: TextAlign.center, style: theme.textTheme.bodyLarge?.copyWith(color: AppColors.textSecondary, height: 1.45)), const SizedBox(height: 18), const Row(children: [Expanded(child: _StateBadge(title: 'LOCAL SOS', value: 'Active', good: true)), SizedBox(width: 10), Expanded(child: _StateBadge(title: 'OFFLINE GPS', value: 'Tracking', good: true))]), const SizedBox(height: 18), SizedBox(width: double.infinity, height: 52, child: FilledButton.icon(onPressed: () { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Connection restored locally.'))); context.go('/home'); }, icon: const Icon(Icons.refresh_rounded), label: const Text('Retry Connection'))), const SizedBox(height: 10), SizedBox(width: double.infinity, height: 50, child: OutlinedButton.icon(onPressed: () => context.go('/setup/contacts'), icon: const Icon(Icons.contact_phone_outlined), label: const Text('Emergency Contacts'))), const SizedBox(height: 16), Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), decoration: BoxDecoration(color: const Color(0xFFFFF1F1), borderRadius: BorderRadius.circular(999)), child: Text('Guardian Active Offline', style: theme.textTheme.labelSmall?.copyWith(color: AppColors.emergencyRed, fontWeight: FontWeight.w700)))]),
                    ),
                  ],
                ),
              ),
