                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      Text('Sarah Jenkins', style: theme.textTheme.headlineMedium?.copyWith(color: Colors.white, fontWeight: FontWeight.w700)),
                      const SizedBox(height: 6),
                      ...
                      const Spacer(),
                      const Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [_CallAction(icon: Icons.volume_up_outlined, label: 'Speaker'), _CallAction(icon: Icons.mic_off_outlined, label: 'Mute'), _CallAction(icon: Icons.videocam_outlined, label: 'Video')]),
                      const SizedBox(height: 20),
                  ...
                  child: ListView(
                    ...
                    children: [
                      Container(
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Nearby Safe Points', style: theme.textTheme.headlineSmall?.copyWith(color: AppColors.trustNavy, fontWeight: FontWeight.w700)), const SizedBox(height: 6), Text('Routes to safety', style: theme.textTheme.bodySmall?.copyWith(color: AppColors.textSecondary))]),
                      ),
                      const SizedBox(height: 16),
                      const _SafePointCard(name: 'Central Station', detail: '24/7 Police Presence • Secure Zone', distance: '0.2 miles', primary: true),
                      const SizedBox(height: 12),
                      const _SafePointCard(name: 'City General ER', detail: 'Emergency Medical • Well-lit Area', distance: '0.8 miles'),
                      const SizedBox(height: 12),
                      const _SafePointCard(name: 'Safeway 24/7', detail: 'Safe-Haven Partner • Open Now', distance: '1.1 miles'),
                    ],
                  ),
                ),
                Padding(
...
class _CallAction extends StatelessWidget {
  ...
  final String label;
  @override
  Widget build(BuildContext context) => Container(width: 76, height: 76, decoration: BoxDecoration(color: Colors.white.withOpacity(0.08), borderRadius: BorderRadius.circular(20)), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, color: Colors.white), const SizedBox(height: 8), Text(label, style: Theme.of(context).textTheme.labelSmall?.copyWith(color: Colors.white70, fontWeight: FontWeight.w700))]));
}

class _SafePointCard extends StatelessWidget {
  const _SafePointCard({required this.name, required this.detail, required this.distance, this.primary = false});
  final String name;
  final String detail;
  final String distance;
  final bool primary;
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24), border: Border.all(color: primary ? const Color(0xFF43D49A) : const Color(0xFFDDE5F0), width: primary ? 2 : 1)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [CircleAvatar(radius: 18, backgroundColor: primary ? const Color(0xFFE8FAF1) : const Color(0xFFEAF1FD), child: Icon(primary ? Icons.shield_outlined : Icons.local_hospital_outlined, color: primary ? AppColors.safeGreen : AppColors.skyBlue)), const Spacer(), Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5), decoration: BoxDecoration(color: const Color(0xFFEAF1FD), borderRadius: BorderRadius.circular(999)), child: Text(distance, style: const TextStyle(color: AppColors.skyBlue, fontWeight: FontWeight.w700, fontSize: 10)))]), const SizedBox(height: 12), Text(name, style: Theme.of(context).textTheme.titleLarge?.copyWith(color: AppColors.trustNavy, fontWeight: FontWeight.w700)), const SizedBox(height: 4), Text(detail, style: Theme.of(context).textTheme.bodySmall?.copyWith(color: AppColors.textSecondary)), const SizedBox(height: 14), SizedBox(width: double.infinity, height: 48, child: FilledButton.icon(onPressed: () {}, style: FilledButton.styleFrom(backgroundColor: primary ? AppColors.trustNavy : const Color(0xFFEAF1FD), foregroundColor: primary ? Colors.white : AppColors.trustNavy), icon: const Icon(Icons.navigation_outlined), label: const Text('Get Directions')))]));
}

class _HotlineTile extends StatelessWidget {
  const _HotlineTile({required this.title, required this.subtitle});
  final String title;
  final String subtitle;
  @override
